...
ACLs allow you to define permissions for **specific users or groups** beyond the standard User / Group / Others model.

To view the Access Control List of a specific file:
```bash
getfacl filename
```

To add a special permission for a specific user using ACL:
```bash
# Grant read permission to user1 on the file
setfacl -m u:user1:r filename
```
**Explanation:**

- `setfacl` : Command used to modify ACL entries
    
- `-m` : Modify (add or change) an ACL rule
...
> This does **not** change the default file permissions shown by `chmod`, but adds an additional access rule evaluated by the kernel.

#### Common ACL Arguments
```bash
# Remove ACL permission for a specific user
setfacl -x u:user1 filename

# Remove all ACL entries from a file (reset ACLs)
setfacl -b filename

# Set ACL for a group
setfacl -m g:devgroup:rw filename

# Apply ACLs recursively to a directory
setfacl -R -m u:user1:rx /path/to/directory

# Set default ACLs (inherited by new files inside a directory)
setfacl -d -m u:user1:rx /path/to/directory
```
...
- `-b` : Remove all ACL entries (clear ACLs)
    
- `g` : Group ACL entry
    
- `-R` : Apply ACLs recursively
    
- `-d` : Set default ACLs for directories (inheritance)
    
> **Note:** Default ACLs only work on directories and affect newly created files inside them.
